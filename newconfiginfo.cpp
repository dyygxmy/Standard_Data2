    ...
    ui(new Ui::Newconfiginfo)
{
    ...
    qDebug() << "strUpInversionCon"<<strUpInversionCon;
    ...
    qDebug() << "strUpInversion"<<strUpInversion;
...
{
    query = new QSqlQuery(db);
    qDebug()<<"truncate table "+tablePreview;
    if(!query->exec("truncate table "+tablePreview))
    {
        qDebug()<<"truncate queue Databases fail"<<query->lastError();
        emit sendTruncateQueueResult(false);
    }
    else
    {
        qDebug()<<"truncate queue Databases success";
...
//                        editBox[previouswhichar-buttnumber]  ->setText(configIniRead->value(QString("carinfo").append(QString::number(i)).append("/QRCodeRule")).toString());
    ...
    if(tmpisselect)  ////第一组有选配车型信息
    {
        ...
        for(int j= 1;j< 6;j++)
        {
            editBox[j-1]  ->setText(configIniRead->value(QString("carinfo").append(QString::number(whichcar)).append("/QRCodeRule").append(QString::number(j))).toString());
            for(int i = 0;i<20;i++)
            {
...
{
    //选配按钮
    if(Factory == "SVW3"||Factory=="Ningbo"|| Factory=="SVW2" || Factory=="BYDSZ" )
    {
        if(optionIscheck)
        {
            isedit = 0;
...
//            configIniRead->setValue(QString("carinfo").append(QString::number(whichcar)).append("/QRCodeRule"),ui->editQRCodeRule1->text());
        ...
        else if(isedit == 1)
        {
            ...
            for(int j = 1;j < 6;j++)
            {
                DTdebug() << j  << editBox[j-1]->text() ;
                configIniRead->setValue(QString("carinfo").append(QString::number(whichcar)).append("/QRCodeRule").append(QString::number(j)),editBox[j-1]->text());
...
void Newconfiginfo::on_pushButton_15_clicked()
{
    ...
    if(isedit == 1)
    {
        ...
        }else
        {
            ...
            {
//                QSettings *configIniRead = new QSettings("/config.ini", QSettings::IniFormat);
//                editBox[j-1] ->setText(configIniRead->value(QString("carinfo").append(QString::number(j)).append("/QRCodeRule").append(QString::number(j))).toString());
//                delete configIniRead ;  //to do
    ...
    else if(!isedit)
    {
        ...
        for(i = 0;i<20;i++)
        {
            ...
            if(carStyle!=ui->lineEdit_carname->text() || Vintmp!=ui->lineEdit_VIN->text() || 
...
void Newconfiginfo::on_pushButton_25_clicked()
{
    ...
    else if(SaveWhat == "saveself")
    {
        ...
        for(int j =0;j<4;j++)
        {
            ...
            qDebug()<<PDMxuan_Name[whichoption-1][j];
        }
...
#if 0
        if(ui->upInversioncheck->isChecked())
        {
            configIniRead->setValue("baseinfo/upInversion","true");
        }
        else {
            configIniRead->setValue("baseinfo/upInversion","false");
        }
#else

        configIniRead->setValue("baseinfo/upInversion",ui->upInversioncheck->isChecked());
        qDebug() << "strUpVersion" << ui->upInversioncheck->isChecked();
#endif
...
#else
    aff = "select IDCode,ScrewID,Program,Torque,Angle,CASE `TighteningStatus` WHEN 'LSN' THEN '' ELSE DATE_FORMAT(TighteningTime,'%Y-%m-%d %H:%i:%s') END AS TighteningTime,TighteningStatus,UploadMark,Cycle from "+Localtable+condition+" order by RecordID DESC limit "+thepages +", 10" ;
#endif
...
//            }
        ...
        if(Selectsecond)//  查询全部数据    //查询指定条件的全部数据
        {
            ...
            else
            {
                ...
                qDebug() << "All number: " << Totalnum;//numRows;
            }
        }
        //如果需要百分比则查询全部数据
        ...
        else
        {
            ...
            qDebug() << "select total row number: " << numRows;//总条数
            if(Selectsecond)
            {
                Selectsecond = false;
                if(Totalnum!=0)
                    ...
                    Nok_percent = 0;
             //display Nok_percent
                qDebug()<< "Nok_percent:"<< Nok_percent;
            ...
            //qDebug()<< "aff="<< aff;
#else
            aff = "select IDCode,ScrewID,Program,Torque,Angle,CASE `TighteningStatus` WHEN 'LSN' THEN '' ELSE "
                    "DATE_FORMAT(TighteningTime,'%Y-%m-%d %H:%i:%s') "
                    "END AS TighteningTime,TighteningStatus,UploadMark,Cycle from "
                    +Localtable+condition+" order by RecordID DESC limit "+thepages +", 10";

//                   select IDCode,ScrewID,Program,Torque,Angle,CASE `TighteningStatus` WHEN 'OK' THEN '' ELSE
//                           DATE_FORMAT(TighteningTime,'%Y-%m-%d %H:%i:%s')
//                           END AS TighteningTime,TighteningStatus,UploadMark,Cycle from SVW2TighteningDatas
//                           order by RecordID DESC limit 20 ;
#endif
            DTdebug() << aff ;
            model->setQuery(aff);
...
void Newconfiginfo::on_pushButton_4_clicked()
{
    ...
    if(!isColumnName)
    {
        if(!ui->lineEdit_column->text().isEmpty())
        {
            ...
            qDebug()<<"tablePreview add column "<<column;
            query->exec("select column_name from information_schema.columns where table_name='"+tablePreview+"'");
            for(int i=0;i<6;i++)
                query->next();
            QString columns = "";
...
void Newconfiginfo::on_pushButton_51_clicked()
{
    ...
    while(query1->next())
    {
        if(ui->lineEdit_column_2->text() == query1->value(0).toString())
        {
            ...
            qDebug()<<"tablePreview drop column "<<ui->lineEdit_column_2->text();
            query->exec("select column_name from information_schema.columns where table_name='"+tablePreview+"'");
            for(int i=0;i<6;i++)
                query->next();
            QString columns = "";
...
void Newconfiginfo::on_pushButton_taotong_add_clicked()
{
    ...
    if((Factory == "SVW3"||Factory=="SVW2")&&(ui->lineEdit_taotong->text().toInt()+1) == 5)
...
void Newconfiginfo::history()
{
    ...
    ui->labelUpdateFlagHint->setText("");
    ui->labelUpdateFlagHint->setVisible(true);

    query = new QSqlQuery(db);
    QString strUpdate = "SELECT COUNT(*) FROM "+Localtable+" where UploadMark = 6 or UploadMark = 7" ;
    ...
    if(result)
    {
        if(query->next())
        {
            ...
            DTdebug() << "not upload data" << count ;
            if(count>0)
            {
                ui->btnUploadData->setEnabled(true);
                ui->btnUploadData->setStyleSheet("QPushButton{font: 14pt \"黑体\";color: rgb(248, 248, 255);border-style:solid;"
                                                             "background-color: rgb(51, 153, 255);}"
                                                 "QPushButton:pressed{Qt::ToolButtonTextUnderIcon}");
            }
            else {
                ui->btnUploadData->setEnabled(false);
                ui->btnUploadData->setStyleSheet("background-color:rgb(245,245,245)");
                ui->labelUpdateFlagHint->setStyleSheet("color:black");
                ui->labelUpdateFlagHint->setText("没有需要补传的数据");
            }
        }
        else {
            DTdebug() << "query without next" ;
        }
    ...
    QTimer::singleShot(1000,this,SLOT(slotCloseHint()));
...
void Newconfiginfo::on_Led_red_clicked()
{
    ...
    */
    ...
    //    QFile taotong2("/sys/class/gpio/gpio149/value");
    ...
    //    qDebug()<< "****test red*****" << k1 << k2;
    //    if( (k1 == 0)&&(k2 == 0) )
    //    {
    //        ui->label_124->setStyleSheet("background-color: rgb(255, 0, 0);color: rgb(248, 248, 248);font: 14pt \"黑体\";");
    //    }
...
void Newconfiginfo::on_Led_yellow_clicked()
{
    ...
    //    usleep(30000);                                              //about 30ms
    //    QFile taotong1("/sys/class/gpio/gpio164/value");
    //    taotong1.open(QIODevice::ReadOnly);
    //    QTextStream in1(&taotong1);
    //    in1 >> k1;
    //    taotong1.close();
...
void Newconfiginfo::on_Led_white_clicked()
{
    ...
    //    usleep(30000);                                              //about 30ms
    //    QFile taotong1("/sys/class/gpio/gpio159/value");
    //    taotong1.open(QIODevice::ReadOnly);
    //    QTextStream in1(&taotong1);
    //    in1 >> k1;
    //    taotong1.close();
...
void Newconfiginfo::receiveRepairReply(int errorCode, QVariantMap replyMap)
{
    if(errorCode==0)
    {
        ...
        else
        {
            ...
            qDebug()<< "Get repair fail errcode:"<<replyMap["ErrorCode"].toString();
        }
    }
    else if(errorCode == 1)
    {
...
void Newconfiginfo::on_pushButton_repairUpload_clicked()
{
    if(ui->label_repairVin->text().isEmpty()||ui->tableWidget_repair->rowCount()==0)
        return;
    bool isUpload = true;
    qDebug()<<"repairDataList"<<repairDataList<<repairDataList.size();
    for(int i=0;i<repairDataList.size();i++)
    {
...
void Newconfiginfo::on_btnUploadData_clicked()
{
    //一键补传未上传数据（标志位6/7）
    query = new QSqlQuery(db);
    qDebug()<<"补传数据"+Localtable;

    ui->labelUpdateFlagHint->setVisible(true);

    QString strUpdate = "UPDATE "+Localtable+" SET UploadMark = 4 where UploadMark = 6 or UploadMark = 7" ;
    bool result = query->exec(strUpdate) ;

    if(result)
    {
        qDebug()<<"update flag success";
        ui->labelUpdateFlagHint->setStyleSheet("color:green;");
        ui->labelUpdateFlagHint->setText("补传请求提交成功，开始补传...");
        ui->btnUploadData->setEnabled(false);
        ui->btnUploadData->setStyleSheet("background-color:rgb(245,245,245)");
    }
    else {
        qDebug()<<"update flag fail"<<query->lastError();
        ui->labelUpdateFlagHint->setStyleSheet("color:red;");
        ui->labelUpdateFlagHint->setText("补传请求失败，请重新补传");
    }
    QTimer::singleShot(1000,this,SLOT(slotCloseHint()));
}

void Newconfiginfo::slotCloseHint()
{
    ui->labelUpdateFlagHint->setVisible(false);
}
