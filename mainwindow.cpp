...
//    progressBarAll  = false ;
    ...
    if((ControlType_1 == "SB356")||(variable1 == "op")||(variable1 == "xml"))
    {
        if(Factory != "BAIC" && Factory != "Haima")
        {
            initButton();
        }
    }
    ...
    ui->btnSkipOne->setVisible(false);
    ...
    green_led(0);
    white_led(0);
    if(ControlType_1=="CS351" && Factory!="BYDSZ" && Factory!="BYDXA")
    {
    ...
    onlineCheck     = configIniRead->value("baseinfo/onlineCheck").toBool() ;
    manualInputCode = configIniRead->value("baseinfo/manualInputCode").toBool() ;
    int vinLen = configIniRead->value("baseinfo/vinLen").toInt();
    ...
    if(manuslOperationAdjust && IsQueue)
    {
    ...
    if(Factory=="BYDSZ"||Factory=="BYDXA"||Factory=="Haima")
    {
        ...
        {
            VINhead = vinHead ;
            ui->label_17->setStyleSheet("font: 35pt \"黑体\";background:transparent;");
        }
        else {
            VINhead="LC0";
            vinAttributeBit = 3;
                    ...
                    else
                    {
                        ...
                        for(j = 0;j< 20;j++)
                        {
                            if(carInfor[n].boltNum[j]>0)//bolt number
                            {
                                ...
                                {
                                    condition = (carInfor[n].ttNum[j] >= 1 && carInfor[n].ttNum[j] <= 4);
                                }
                                else if (Factory == "Ningbo"||Factory == "BYDSZ"||Factory == "BYDXA" || Factory == "Haima")  // new haima todo
                                {
                                    condition = (carInfor[n].ttNum[j] >= 1 && carInfor[n].ttNum[j] <= 8);
                                }
                                ...
                                else
                                {
                                    if(allChannel==1)
                                    {
                                        ui->label_pronum->setText(carInfor[n].proNo[j]);
                                        ui->label_lsnum->setText(QString::number(carInfor[n].boltNum[j]));
                                    }
                                    if(n == 0)
                                    {
                                    ...
                                    {
                                        emit sendOperate2(true,j);
                                        DTdebug()<<"*******send enable ch2*********group:"<<j;
                                    }
                                    else if(n == 2)
                                    {
                                        emit sendOperate3(true,j);
                                        DTdebug()<<"*******send enable ch3*********group:"<<j;
                                    }
                                    else
                                    {
                                        emit sendOperate4(true,j);
...
void MainWindow::searchQRCodeNum(QString strQRCode)
{
    ...
    if(flagQR)
    {
        ...
        {
            emit sendOperate1(true,groupNumCh[preChannel]);
        }
        else if(preChannel == 1)
        {
            emit sendOperate2(true,groupNumCh[preChannel]);
        }
        else if(preChannel == 2)
        {
            emit sendOperate3(true,groupNumCh[preChannel]);
        }
        else
        {
            emit sendOperate4(true,groupNumCh[preChannel]);
        }
#endif
    }
    else {
...
void MainWindow::searchVinNum()
{
    ...
    for(i = 1;i < 101;i++)    //100个车型
    {
        ...
        if(Factory == "Haima" && StationName.contains("分装线"))
        {
            ...
            if(len >serialNums.length() || temp.isEmpty())
            {
                continue ;
            }
        }
    ...
    if(Factory == "BAIC" && onlineCheck)
    {
        ...
        if(db.isOpen() && QSqlDatabase::contains("mysqlconnectionmatch"))
        {
            ...
            if(query.next())  //查到了匹配
            {
                carTypeBAIC = query.value(0).toString() ;
                DTdebug()<<"select cartype success"<< carTypeBAIC;

                for(i = 1;i < 101;i++)    //100个车型
                {
                    carType = config->value(QString("carinfo").append(QString::number(i)).append("/carcx")).toString();
                    if(carType == carTypeBAIC)
                    {
                        whichar = i ;
                        flags = true ;
                        DTdebug() << whichar << carType;
...
{
//    DTdebug()<<"pinCode mainwindows  pinBuf:"<< pinBuf <<"len:"<<len;
...
{
//    DTdebug()<<"pinCode pinCodeRightVerify  pinBuf1:"<< pinBuf1 <<"len:"<<len;
    int sum,sumOdd,sumEven,result,i;
    QByteArray pinTemp;
...
int MainWindow::VIN_VerifyFunc( char *buf)
{
    ...
    if(Factory=="BAIC"||Factory == "GZBAIC"||Factory == "GM" ||Factory == "Haima" ||isReplaceBarcode ||(Factory == "Dongfeng" && manualMode))   //GM VIN not need verify
    {
        return 1;
    }
...
void MainWindow::handleCodeBarcode(QString code,bool invalidFlag)
{
    ...
    else
    {
        ...
        if(Factory == "BAIC" && ControlType_1 == "SB356")  //add by wxm  //to do  考虑北汽轮胎机模式
        {
            cleanDisplay();
        }
        //不重复 校验
            ...
            if(tempp=="VIN")
            {
                ...
                if(SaveWhat == "delete_car")     //删除车型
                {
                    if(serialNums == VIN_PIN_SQL)
                    {
                        ...
                        {
                            DTdebug() << "ING" ;
                            emit sendOperate1(false,groupNumCh[0]);
                        }
                    }
...
{
    if(vin.isEmpty() && manuslOperationAdjust)
    {
...
void MainWindow::fromsecondthreaddata(QString MI,QString WI,QString IsOk,int ch,int groupNum)
{
    ...
    FUNC() ;
    DTdebug() <<whichpdmnumnow[ch] << tempnumdpm[ch];
    ...
    else
    {
        ...
        if(IsOk== "Reconnect")  //重新连接
        {
            if(carInfor[0].boltNum[groupNumCh[0]]>0)
            {
                ...
                emit sendOperate1(true,groupNumCh[0]);
                return;
            }
    ...
    else  //本次螺栓合格ok
    {
        ...
        if(carInfor[ch].boltNum[groupNum] == 0)
        {
            ...
            else
            {
                for( k = groupNum;k <20;k++)
                {
                    ...
                    if(carInfor[ch].boltNum[k])
                    {
                        ...
                        if(Factory=="SVW3" ||Factory == "SVW2")
                        {
                            condition = (carInfor[ch].ttNum[k] >= 1 && carInfor[ch].ttNum[k] <= 4);
                        }
                        else if(Factory == "Ningbo"|| Factory == "BYDSZ"|| Factory == "BYDXA")
                        {
                            condition = (carInfor[ch].ttNum[k] >= 1 && carInfor[ch].ttNum[k] <= 8);
                        }
                        if(condition)
                        {
                            TaoTongState = true;
                    ...
                    if(carInfor[ch].boltNum[k])
                    {
                        ...
                        {
                            emit sendOperate2(true,k);
                        }
                        else if(ch == 2)
                        {
                            emit sendOperate3(true,k);
                        }
                        else
                        {
                            emit sendOperate4(true,k);
                        }
                        break;
                    }
                }
            ...
            if(k == 20)
            {
                ...
                {
                    emit sendOperate2(false,k);
                }
                else if(ch == 2)
                {
                    emit sendOperate3(false,k);
                }
                else
                {
                    emit sendOperate4(false,k);
                }
        ...
        else
        {
            if(MI == "-1" && WI == "-1") //more bolt number
            {
                ...
                {
                    emit sendOperate2(true,groupNum);
                }
                else if(ch == 2)
                {
                    emit sendOperate3(true,groupNum);
                }
                else
                {
                    emit sendOperate4(true,groupNum);
                }
            }
        }
    }
            ...
            if(groupAllBoltNumCh[0]>PDMBoltNum)
            {
                ...
                for( i = whichpronumis-1;i<20;i++)
                {
                    ...
                    if(carInfor[0].boltNum[i])
                    {
                        ui->label_pronum->setText(carInfor[0].proNo[i]);
                        ui->label_lsnum->setText(QString::number(PDMBoltNum));
                        emit sendOperate1(true,StartBolt);
                        break;
                    }
                    else
                    {
...
void MainWindow::fromsecondthreaddata(QString MI1,QString WI1,QString IsOk1,QString MI2,QString WI2,QString IsOk2)
{
    ...
    else  //本次螺栓合格ok
    {
        ...
        ui->label_hege->setText(QString::number(QualifiedNum));
        ui->label_lsnum->setText(QString::number(carInfor[0].boltNum[whichpdmnumnow[0]]));
        nok_led(0);
        whichpdmnumnow[0]++;  //OK
        PDMBoltBuf[0] = 1; //ING
        PDMBoltBuf[1] = 1; //ING
    ...
    if(rfidNextCom && Factory != "Dongfeng")    //数据库不记录全部确认的数据
    {
        ...
        else
        {
            if(VIN_PIN_SQL_RFID_previous != "")
            {
                ...
                G9tmpp = VIN_PIN_SQL_RFID_previous.right(4);  //modify by wxm 2020/3/30
                resetUiDo();
            ...
            G9tmpp = VIN_PIN_SQL_RFID.right(4);   //modify by wxm 2020/3/30
        }
    ...
    else
    {
        ...
        else
        {
            ...
            if(allChannel == 1)//single channel
            {
                ...
                emit sendOperate1(false,98);//job batch incerment
...
void MainWindow::configwarning(bool isconfigone)
{
    ...
    DTdebug()<< isconfigone << nextCarCatch<< isSaveShow ;
    ConfigOneOrAll = isconfigone;
//    progressBarAll = progressBarStop ;


    if(!isconfigone || !nextCarCatch /*|| progressBarStop*/)
    {
        if(factory == "GZBAIC")
        {
            return ;
...
void MainWindow::taotong_main(int  which_t)
{
    ...
    if(preChannel!=-1)
    {
        ...
        if(carInfor[preChannel].ttNum[groupNumCh[preChannel]] == which_t || which_t == 50)
        {
        ...
        else
        {
            ...
            if((sendEnableFlag)||(ttChangeFlag))
            {
                ...
                if(carInfor[preChannel].boltNum[groupNumCh[preChannel]])
                {
                    ...
                    {
                        emit sendOperate1(false,groupNumCh[preChannel]);
                    }
                    else if(preChannel == 1)
                    {
                        emit sendOperate2(false,groupNumCh[preChannel]);
                    }
                    else if(preChannel == 2)
                    {
                        emit sendOperate3(false,groupNumCh[preChannel]);
                    }
                    else
                    {
                        emit sendOperate4(false,groupNumCh[preChannel]);
                    }
                }
            }
        }
...
void MainWindow::receiveGetCar()
{
    if(SYSS=="ING")
    {
        if(Factory != "Dongfeng")
        {
            ...
            emit sendOperate1(false,groupNumCh[0]);
            emit sendOperate2(false,groupNumCh[1]);
        }
        else
        {
            risingEdge();
...
void MainWindow::on_pushButton_16_clicked()
{
    if((IsQueue && !controlMode && (Factory =="BYDSZ"||Factory =="BYDXA")) || (Factory == "Dongfeng" && !manualMode)||m_BAICQueue)
    {
    ...
    else if ( manualInputCode && SYSS != "ING"  && CsIsConnect ) {
        e3 = new QGraphicsOpacityEffect(this);
        e3->setOpacity(0.5);
        ui->label_black->setGraphicsEffect(e3);
        ui->label_black->show();
        ...
        DTdebug() << manualInputCode << SYSS << CsIsConnect ;
    }
#if 0
    //test 模拟接受条码
    else {
        getSerialNum("LSVWY2183HN274481",false,"BarCode");
    }
//    slotGetQueueNum("LMVHFEFZ6JA880074","test");
...
{
    recvClose();
    getSerialNum(code,false,"BarCode");
}
    ...
    ************************************************************************/
    if(preSystemStatus != systemStatus)
    {
        ...
        case 23:
            ...
            break;
        case 24:
            ui->label_tiaomastate->setText(tr("服务器断开连接"));
            break;
        default:
            ui->label_tiaomastate->setText("");
            break;
...
void MainWindow::on_pushButton_tiaoma_clicked()
{
    if(Factory == "BYDSZ")
    {
        if(!controlMode)
        {
            ...
            emit sendOperate1(false,99); //manual mode
            cleanDisplay();
            PDM_PATH="";
        ...
        {
            emit sendOperate1(false,1); //auto mode
...
void MainWindow::initButton()
{
    ...
    for(int i=0;i<3;i++)
    {
        for(int j = 0;j<10;j++)
        {
            ...
            pushButton_channel[i][j]->setGeometry(30,35+56*j,44,44);
            label1_channel[i][j]->setGeometry(84,34+56*j,150,46);
            label2_channel[i][j]->setGeometry(234,34+56*j,150,46);
            label1_channel[i][j]->setStyleSheet("border-style:solid;border-color: rgb(51, 153, 255); border-width:0px;font: 18pt \"黑体\";");
            label2_channel[i][j]->setStyleSheet("border-style:solid;border-color: rgb(51, 153, 255); border-width:0px;font: 18pt \"黑体\";");
...
void MainWindow::fromFtpJsonData(QString MI, QString WI, QString STATE, int Channel, int Screwid, int Program)
{
    ...
    if(STATE=="OK")
    {
        pushButton_channel[Channel-1][num]->setStyleSheet("border-image: url(:/LCD_CS351/LCD_CS351/35_all/g01.png);font: 20pt \"黑体\";color: rgb(248, 248, 255);");
    }
    else if(STATE=="NOK")
    {
        pushButton_channel[Channel-1][num]->setStyleSheet("border-image: url(:/LCD_CS351/LCD_CS351/35_all/r01.png);font: 20pt \"黑体\";color: rgb(248, 248, 255);");
    }
...
{
    if(ControlType_1 =="SB356" && Factory != "BAIC" && Factory != "Haima")
    {
        if(sb356connect)
        {
            systemStatus = 0;
    ...
    else if(ControlType_1 =="SB356_PLC" ||(ControlType_1 =="SB356" && (Factory =="BAIC" || Factory == "Haima")))
    {
        if(sb356connect)
        {
            if(SYSS != "ING" && SYSS != "OK" && SYSS != "NOK")
...
{
    emit sendOperate1(false,97);//job off
}
