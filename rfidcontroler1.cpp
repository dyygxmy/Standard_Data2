...
{
    m_pTcpSocket->abort();                        //取消已有的连�?
    m_pTcpSocket->connectToHost(rfidip,rfidport); //连接到主�?
...
void RfidControler::receivemss()
{
        ...
        if(revBuf.size() >= 31)
        {
            if( ((unsigned char)revBuf[0] == 0x02)&&((unsigned char)revBuf[30] == 0x03) )
            {
                if((unsigned char)revBuf[3] == 0x31)//pin code
                {
                    ...
                    tempG9  = QString(revBuf.mid(18,4));     //modify 2020-03-31
    ...
    else //master and other
    {
        ...
        else
        {
            ...
            tempG9  = pin.mid(16,4);    //modify 2020-3-30
            DTdebug() << "************RFID***********"<<tempPin<< tempG9;
            if(tempPin== "" || tempG9 == "")
            {
        ...
        else
        {
            ...
            {
                DTdebug()<<"************error*************"<< tempPin << tempG9 << controllerStatusFlag << workmode << isRFID << (systemRunStatusFlag && RFIDIngNotIn);
            }
            else
            {
                isEqual = pinCodeRepeatHandle(tempPin);
                ...
                {
                    DTdebug() << "PIN code valid:"  << tempPin << tempG9;
