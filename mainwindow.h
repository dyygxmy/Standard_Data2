...
//    QTimer timer_showdown;
    ...
    QLabel *label1[4][50];
    QLabel *label2[4][50];
    QWidget *widget_channel[3];
    QPushButton *pushButton_channel[3][10];
    QLabel *label1_channel[3][10];
    QLabel *label2_channel[3][10];

    QPushButton *btnCarType[CAR_TYPE_AMOUNT] ;  //用于广州北汽选择车型

    int Channel_num[3];
    int Channel_Screwid[3];
    int Channel_time[3];
...
public:
    ...
    bool manualInputCode ;
    bool onlineCheck ; //北汽通过数据库校验，其他工厂应该可以用
};
