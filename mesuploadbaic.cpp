...
//                                                query2.prepare("insert into "+tablePreview+" (VIN,carType,tolNumber,UseFlag,LocalDateTime) values (?, ?, ?, 0, now())");
...
#endif
                                                    query2.exec("delete from "+tablePreview+" where RecordID in (select x.RecordID from (select RecordID from "+tablePreview+" order by RecordID limit  100) as  x)");
                                                }
                                                else {
                                                ...
                                                DTdebug() <<"update FisPreview : "<<value["vin"].toString()<<value["carType"].toString()<<value["tolNumber"].toString(); //add by wxm 2020/5/21
                                                if(!query2.exec())
